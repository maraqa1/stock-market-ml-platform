...
STOCKML_ALPACA_MAX_ORDERS=20
STOCKML_ALPACA_MAX_NOTIONAL_PER_ORDER=5000
STOCKML_ALPACA_MAX_TOTAL_NOTIONAL=50000
STOCKML_MAX_POSITION_PCT=0.05
STOCKML_CANDIDATE_POOL_SIZE=200
...
These defaults describe a higher-risk paper profile for an account near $100k: up to 20 planned names, up to $5k per order, and up to $50k total planned basket notional. To make an existing VM use this profile, update `/etc/stockml/stockml.env`; repository defaults do not override values already present in that environment file.
