...
class CandidateValidityGateBlock(PlaceholderBlock):
    ...
    def evaluate(self, payload: dict | None = None) -> BrainBlockResult:
        ...
        return BrainBlockResult(
            ...
            details={"valid_candidates": len(result.valid_candidates), "invalid_candidates": len(result.invalid_candidates)},
        )
    ...
    def validate_candidate(self, candidate: Candidate) -> tuple[str, ...]:
        ...
        if not candidate.side or candidate.side not in SUPPORTED_SIDES:
            reasons.append("side_missing_or_unsupported")
        if str(candidate.candidate_status or "").strip().lower() != "executable":
        ...
        if not candidate.price_check_clear:
    ...
    def validate_candidates(self, candidates: list[Candidate]) -> CandidateValidityResult:
        ...
        return CandidateValidityResult(valid_candidates=valid, invalid_candidates=invalid)
    ...
    def validate_normalization_result(self, normalization: CandidateNormalizationResult) -> CandidateValidityResult:
        ...
        return CandidateValidityResult(valid_candidates=result.valid_candidates, invalid_candidates=invalid)
