...
def build_metadata_enriched(limit: Optional[int] = None, sleep_seconds: float = 0.25) -> Dict[str, Path]:
    ensure_data_dirs()
    stamp = timestamp()
    universe_path = latest_validated_universe_file()
    universe = pd.read_csv(universe_path, dtype=str)
    metadata = fetch_metadata_for_universe(universe, limit=limit, sleep_seconds=sleep_seconds)
...
def main() -> int:
    ...
    paths = build_metadata_enriched(limit=args.limit, sleep_seconds=args.sleep_seconds)
